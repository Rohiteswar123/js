const typeColor = {
  ...
  let getPokeData = async () => {
    ...
    //     generateCard(data);
    //   });
     var json1 =await fetch(finalUrl);
     var data= await json1.json()
     generateCard(data)
     c++;
  };
  let getPokeData1 = async () => {
    // Generate a random number between 1 and 150
  ...
  btn1.addEventListener("click", getPokeData1);
   setInterval(() => {
    if(c1>c2){
        alert("C2 wins");
  ...
  window.addEventListener("load", getPokeData1);